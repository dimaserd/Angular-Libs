...
import { LoginService, PrivateFileUploadService } from 'croco-generic-app-logic';
...
export class ChatLogicService {
  ...
  public editableMessage$: BehaviorSubject<MessageView | undefined | null> = new BehaviorSubject(null);
  ...
  initChatLogic(chatId: number) {
    ...
    if (!this._loginDataSubscription) {   
      ...
      this._loginDataSubscription = this._loginService.getLoginDataCached().subscribe(data => {
    ...
    this._updateMessagesSubscription = this.updateMessages$.pipe(switchMap(() => {
  ...
  public sendAfterEdit(messageText: string): void {
    if (messageText) {
      ...
      this._chatMessagingService.edit({
        message: messageText,
        id,
      })
      .subscribe(() => {
  ...
  public setUploadFilesWithTextRequest(
    messageText: string,
    files: File[],
    filesUploadingProgress: BehaviorSubject<{ uploadingLoaded?: number, uploadingTotal?: number }[]>,
    ...
    return this.privateFileUploadService
      .uploadWithProgress(fileList, true, null).pipe(
      tap((result) => {
  ...
  private getChatRootDataChanges(chatId: number): Observable<ChatDetailedModel> {
    return this._chatService.getById(chatId).pipe(
      tap(data => { this.chatUsers = data.users; }),
      distinctUntilChanged(
        (a, b) =>
          a.id === b.id &&
  ...
  public getLastMessages(): Observable<{ messages: MessagesDayGroupInterface[]; newIncomingMessages?: MessageView[] }> {
    ...
    return timer(0, this.chatSettings.lastMessagesUpdateInterval).pipe(
      ...
      map((lastMessages) => {
        const messagesToAdd = mapAndFilterMessagesBeforePush(lastMessages.list, messages);
        messages = pushMessages(
          messagesToAdd,
          messages,
          this.chatUsers.map(item => item.user),
          messages[0]?.messagesGroups[0]?.messages[0]?.message.sentOnUtcTicks > messagesToAdd[0]?.message.sentOnUtcTicks,
          this.currentUserId
        );

        const newIncomingMessages = messagesToAdd.filter(({ message }) => message.senderUserId !== this.currentUserId);

        if (newIncomingMessages?.length) {
          this.notReadMessages = [...this.notReadMessages, ...newIncomingMessages];
        }
  ...
  public getMessagesAfterScroll(messages: MessagesDayGroupInterface[]): Observable<{ messages: MessagesDayGroupInterface[]; newIncomingMessages?: MessageView[] }> {
    return this._chatMessagingService
      .list({
        chatId: this.chatIdSubject$.getValue(),
        count: 30,
        lessThantUtcTicks: messages[0]?.messagesGroups[0].messages[0].message.sentOnUtcTicks,
  ...
  public getFirstAndLastVisibleMessages(targetElement: ElementRef, messagesElements: QueryList<MessageComponent>): boolean {
    ...
    if (lastVisibleMessage) {
      const lastVisibleMessageTime = lastVisibleMessage.message?.message.sentOnUtcTicks;
