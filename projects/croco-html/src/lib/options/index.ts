...
export interface CrocoHtmlImageOptions {
  ...
  /**
   * Максимальная высота изображения в предпросмотре (FileImageTagViewComponent) в пикселях (например, 200).
   * Если указано, изображения в предпросмотре не будут превышать эту высоту.
   * Если не указано, используется значение по умолчанию.
   */
  visualViewMaxHeight?: number;
}
